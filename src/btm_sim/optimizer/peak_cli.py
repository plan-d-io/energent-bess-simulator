"""Command-line entry point for the peak-reduction-first LP."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="btm-peak-reduction",
        description=(
            "Best-case peak-reduction optimization using the complete year in "
            "advance. Charging is from PV that would otherwise be exported. "
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory for peak_reduction_dispatch.csv and peak_reduction_summary.json",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    try:
        result = optimize_peak_reduction(
