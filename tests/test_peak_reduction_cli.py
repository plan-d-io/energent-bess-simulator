...
from btm_sim.optimizer.peak_cli import main

UTC = timezone.utc


def test_peak_reduction_cli_writes_outputs(tmp_path: Path):
    ...
    assert code == 0
    summary = json.loads((out / "peak_reduction_summary.json").read_text(encoding="utf-8"))
    assert (out / "peak_reduction_dispatch.csv").exists()
    assert summary["ok"] is True
    assert summary["case"] == "peak_reduction_first"
    ...
    assert list(pd.read_csv(out / "peak_reduction_dispatch.csv").columns[: len(CANONICAL_COLUMNS)]) == list(
        CANONICAL_COLUMNS
    )
